...
Grid::Grid( GRID_VAL val0, GRID_VAL val1, GRID_VAL val2, GRID_VAL val3,
            GRID_VAL val4, GRID_VAL val5, GRID_VAL val6, GRID_VAL val7,
            GRID_VAL val8 ) {
	gridVal_[ 0 ] = val0;
	gridVal_[ 1 ] = val1;
	gridVal_[ 2 ] = val2;
...
bool Grid::MakeStep( int index, GRID_VAL value ) {
	if ( index < 0 || index > 8 ) {
		std::cerr << __FUNCTION__ << "(): Wrong index!\n";
		return false;
	}

	if ( gridVal_[ index ] != GRID_EMPTY ) {
		std::cerr << __FUNCTION__ << "(): Grid value already set!\n";
		return false;
	}
...
Grid Grid::RotateRigth( void ) const {
	Grid retGrid( (GRID_VAL)gridVal_[ 6 ], (GRID_VAL)gridVal_[ 3 ],
	              (GRID_VAL)gridVal_[ 0 ], (GRID_VAL)gridVal_[ 7 ],
	              (GRID_VAL)gridVal_[ 4 ], (GRID_VAL)gridVal_[ 1 ],
	              (GRID_VAL)gridVal_[ 8 ], (GRID_VAL)gridVal_[ 5 ],
	              (GRID_VAL)gridVal_[ 2 ] );
	return retGrid;
}
...
bool Grid::CompareGridsAllRotates( const Grid &grid1, const Grid &grid2,
                                   int &rotation ) {
	// 0 rotate
	if ( grid1 == grid2 ) {
		rotation = 0;
