...
CaseObject::CaseObject( const Pool &pool, const Grid &grid ) {
	pool_ = pool;
	grid_ = grid;
}
...
int Study::GetStepForGrid( const Grid &grid ) {
	...
	for ( size_t i = 0; i < vObjects_.size(); i++ ) {
		if ( Grid::CompareGridsAllRotates( grid, vObjects_[ i ].grid_,
		                                   rotation ) ||
		     Grid::CompareGridsAllRotates(
		         grid.Invert(), vObjects_[ i ].grid_, rotation ) ) {
			std::cout << "Pool: \n";
